...
class Stream(WebRTCConnectionMixin):
    ...
    async def offer(self, body: Body, request: Request):
        """
        ...
        if user_id:
            ...
            try:
                ...
                from src.utils.user_id_storage import set_user_id
                set_user_id(body.webrtc_id, user_id)
                # print(f"✅ 用户ID已存储: webrtc_id={body.webrtc_id}, user_id={user_id}")
            except Exception as e:
                print(f"⚠️ 存储用户ID失败: {e}")
        else:
            # print("⚠️ 未找到用户ID")
            body_dict = body.model_dump()
        
        return await self.handle_offer(
            body_dict, set_outputs=self.set_additional_outputs(body.webrtc_id)
        )
